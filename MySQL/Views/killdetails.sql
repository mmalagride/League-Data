...
from `championkill` ck
left join `friendinfo` ki on ck.event_killerid = ki.summoner_puuid
left join `friendinfo` vi on ck.event_victimid = vi.summoner_puuid
left join `friendinfo` ai1 on ck.event_assist1 = ai1.summoner_puuid
left join `friendinfo` ai2 on ck.event_assist2 = ai2.summoner_puuid
left join `friendinfo` ai3 on ck.event_assist3 = ai3.summoner_puuid
left join `friendinfo` ai4 on ck.event_assist4 = ai4.summoner_puuid
