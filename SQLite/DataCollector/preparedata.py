...
def LoadFreshData(sheetname):
    ...
    gc = pygsheets.authorize(service_file=os.path.dirname(os.path.dirname(os.path.dirname(__file__))) + "\\Secrets\\google-api-key.json")
    ...
    logging.info("Executing fullgamedetails query...")
    dataframe1 = pd.read_sql('SELECT * FROM fullgamedetails where gameStartTime > (strftime(\'%s\', \'now\')*1000) - 7776000000', con=DW['connection'])
    sh3 = gc.open(sheetname + " - 3 Months")
    wks1 = sh3[0]
    wks1.clear()
    wks1.set_dataframe(dataframe1,(1,1))
    logging.info("Executing killdetails query...")
    dataframe2 = pd.read_sql('select kd.* from KillDetails kd left join fullgamedetails fgd on kd.gameID = fgd.gameID where fgd.gameStartTime > (strftime(\'%s\', \'now\')*1000) - 7776000000', con=DW['connection'])
    wks2 = sh3[1]
    wks2.clear()
    wks2.set_dataframe(dataframe2,(1,1))
